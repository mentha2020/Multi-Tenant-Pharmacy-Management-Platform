...
export default function Orders() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-100">My Orders</h1>
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
          ...
          <p className="text-gray-400 dark:text-gray-500 mb-6">Start shopping to see your orders here</p>
          ...
          {orders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                {/* Order Header */}
                <div className="p-4 bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600 flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div className="flex items-center gap-4">
                    <div>
                      <p className="font-mono text-sm text-gray-500 dark:text-gray-400">Order #{order.order_number}</p>
                      <p className="text-sm text-gray-400 dark:text-gray-500">{new Date(order.created_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setSelectedOrder(selectedOrder?.id === order.id ? null : order)} className="text-green-600 dark:text-green-400 hover:text-green-700 text-sm font-medium flex items-center gap-1">
                ...
                <div className="p-4">
                  <div className="flex items-center gap-3 overflow-x-auto pb-2">
                    {order.items?.slice(0, 4).map((item, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded-lg flex-shrink-0">
                    ...
                    {order.items?.length > 4 && (
                      <span className="text-sm text-gray-500 dark:text-gray-400">+{order.items.length - 4} more</span>
                    )}
                  </div>
                  <div className="flex items-center justify-between mt-4 pt-4 border-t dark:border-gray-700">
                    <div className="flex items-center gap-4">
                      <span className="text-sm text-gray-500 dark:text-gray-400">Pharmacy: <span className="font-medium text-gray-700 dark:text-gray-300">{order.pharmacy?.name}</span></span>
                      <span className="text-sm text-gray-500 dark:text-gray-400">Payment: <span className="font-medium text-gray-700 dark:text-gray-300 capitalize">{order.payment_method}</span></span>
                    </div>
