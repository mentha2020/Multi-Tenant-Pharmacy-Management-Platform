    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl shadow-sm">
          ...
          <p className="text-gray-400 mb-6">Start adding medicines from your favorite pharmacies</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {Object.entries(itemsByPharmacy).map(([pharmacyId, { pharmacy, items: pharmacyItems }]) => (
              <div key={pharmacyId} className="bg-white rounded-xl shadow-sm overflow-hidden">
                <div className="p-4 bg-gray-50 border-b flex items-center gap-2">
                  <Store className="h-5 w-5 text-green-600" />
                  <span className="font-semibold text-gray-800">{pharmacy?.name || 'Unknown Pharmacy'}</span>
                </div>
                {pharmacyItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 p-4 border-b last:border-b-0">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-9 h-9 bg-gray-200 rounded-lg flex items-center justify-center hover:bg-gray-300 transition-colors">
            ...
            <div className="flex justify-between items-center">
              <button onClick={() => navigate('/')} className="flex items-center gap-2 text-gray-600 hover:text-green-600 transition-colors">
